...
<Aside type='note' title='Note on Sparse GSIs'>
For GSI Sparse Indexes, if the partition key is `undefined`, the sort key builder will not be called. This is because both keys are required to create the index, so if the partition key is missing, there's no need to compute the sort key.
</Aside>
